...
- 数据完整性：具备一定的完整性验证、数据保护等能力，使正常存取过程中软件不因数据库中异常数据而崩溃，在异常情况下数据库不致损坏。
- 数据库安全：具备一定程度防范SQL注入攻击等常见攻击的能力。
...
- 法律保护：软件工程开发小组保证在软件开发、管理的全过程中，遵守有关计算机安全、计算机软件著作权及用户合法权益的法律法规。